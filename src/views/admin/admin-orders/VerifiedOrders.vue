          ...
          <v-layout :class="`pa-3 order ${order.status}`">
            <v-row>
              ...
              <v-col cols="6" md="4">
                ...
                <v-chip small class="white--text" color="yellow">{{order.status}}</v-chip>
              </v-col>
...
import DeliverOrder from "./admin-orders-dialog/DeliverOrder";
export default {
  name: "verifiedOrders",
  components: { DeliverOrder },
  ...
  created() {
    this.loading = true;
    let orderRef = db.collection("verifiedOrders").orderBy("timestamp", "asc");
